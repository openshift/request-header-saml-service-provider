...
This error is an indication that your SAML mappings in Keycloak are not mapping correctly. Ensure that the property `username` from keycloak is mapped to the SAML Attribute Name `user`. This involves setting a mapper in the client in Keycloak. See [testing_with_keycloak.md](keycloak/testing_with_keycloak.md#mapping-the-data-from-keycloak-to-mod_auth_mellon)
